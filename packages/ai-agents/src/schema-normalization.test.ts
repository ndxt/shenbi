...
describe('normalizeGeneratedNode', () => {
  ...
  it('repairs page schemas and returns repair summaries', () => {
    const repaired = repairPageSchema({
      id: 'page-1',
      body: {
        id: 'body-root',
          ...
          children: [
            {
              id: 'body-root',
              component: 'Alert',
              props: { title: '提示', message: '会被保留为 message' },
            },
          ],
        },
      },
    ...
    const body = repaired.schema.body as SchemaNode;
    expect(body.component).toBe('Container');
    const alertNode = (body.children as SchemaNode[])[0];
    expect(alertNode?.component).toBe('Alert');
    expect(alertNode?.props?.message).toBe('会被保留为 message');
    expect(Array.isArray(repaired.repairs) ? repaired.repairs.length : 0).toBeGreaterThanOrEqual(0);
  });

  it('validates array block roots by wrapping them into a container', () => {
    const normalized = validateGeneratedBlockNode([
      { id: 'a', component: 'Card', props: { title: 'A' }, children: [] },
      { id: 'b', component: 'Card', props: { title: 'B' }, children: [] },
    ], 'summary-block');
