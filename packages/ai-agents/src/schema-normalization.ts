...
interface PageSchemaSanitizationDiagnostic {
  path: string;
  diagnostic: SanitizationDiagnostic;
}

function ensureUniqueAcrossSchemaRoots(schema: PageSchema): void {
  const seen = new Set<string>();
  const visit = (value: unknown, fallbackBase: string): void => {
    if (Array.isArray(value)) {
      value.forEach((item, index) => visit(item, `${fallbackBase}-${index + 1}`));
      return;
    }
    if (!isNodeLike(value)) {
      return;
    }
    ensureUniqueNodeIds(value, seen, fallbackBase);
  };

  visit(schema.body, 'body');
  visit(schema.dialogs, 'dialog');
  if (schema.templates && typeof schema.templates === 'object') {
    Object.entries(schema.templates).forEach(([key, value]) => visit(value, `template-${key}`));
  }
}

function toRepairEntry(
  path: string,
  diagnostic: SanitizationDiagnostic,
): NonNullable<RepairSchemaResult['repairs']>[number] {
  return {
    path,
    message: `${diagnostic.componentType}.${diagnostic.propPath}: ${diagnostic.action} (${diagnostic.rule})`,
  };
}

function normalizeSchemaNodeCollection(
  value: SchemaNode | SchemaNode[] | undefined,
  path: string,
): {
  value: SchemaNode | SchemaNode[] | undefined;
  diagnostics: PageSchemaSanitizationDiagnostic[];
} {
  if (!value) {
  ...
  if (Array.isArray(value)) {
    const diagnostics: PageSchemaSanitizationDiagnostic[] = [];
    const normalized = value.map((node, index) => {
      const result = normalizeGeneratedNodeWithDiagnostics(node);
      diagnostics.push(
  ...
  const result = normalizeGeneratedNodeWithDiagnostics(value);
  return {
    value: result.node,
...
export function repairPageSchemaWithDiagnostics(schema: PageSchema): {
  schema: PageSchema;
  diagnostics: PageSchemaSanitizationDiagnostic[];
  repairs: NonNullable<RepairSchemaResult['repairs']>;
} {
  const bodyResult = normalizeSchemaNodeCollection(schema.body, 'body');
  const dialogsResult = normalizeSchemaNodeCollection(schema.dialogs, 'dialogs');
  const templateEntries = Object.entries(schema.templates ?? {});
  const templateDiagnostics: PageSchemaSanitizationDiagnostic[] = [];
  const templates = templateEntries.length > 0
    ? Object.fromEntries(templateEntries.map(([key, value]) => {
      const result = normalizeSchemaNodeCollection(value, `templates.${key}`);
      templateDiagnostics.push(...result.diagnostics);
      return [key, result.value as SchemaNode];
    }))
    : schema.templates;

  const normalized: PageSchema = {
    ...schema,
    body: (bodyResult.value ?? schema.body) as PageSchema['body'],
    ...(dialogsResult.value ? { dialogs: dialogsResult.value as SchemaNode[] } : {}),
    ...(templates ? { templates } : {}),
  };

  ensureUniqueAcrossSchemaRoots(normalized);

  const diagnostics = [
    ...bodyResult.diagnostics,
    ...dialogsResult.diagnostics,
    ...templateDiagnostics,
  ];
  ...
  return {
    schema: normalized,
    diagnostics,
    repairs: diagnostics.map(({ path, diagnostic }) => toRepairEntry(path, diagnostic)),
  };
}

export function repairPageSchema(schema: PageSchema): RepairSchemaResult {
  const result = repairPageSchemaWithDiagnostics(schema);
  return {
