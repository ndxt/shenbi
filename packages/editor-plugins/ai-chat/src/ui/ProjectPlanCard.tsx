...
function getActionClass(action: 'create' | 'modify' | 'skip'): string {
  switch (action) {
    case 'create':
      return 'text-primary border border-primary/30 bg-primary/10 shadow-sm';
    case 'modify':
      ...
      {!embedded && (
        <div className="flex items-start justify-between gap-2 px-1">
          <div className="flex min-w-0 flex-1 flex-wrap items-start gap-x-3 gap-y-1">
            ...
            {showInlineStatus && (
              <>
                <span className={`shrink-0 ${phase === 'error' ? 'text-red-400' : 'text-primary'}`} style={{ fontSize: '11px' }}>
        ...
        {groupedPages.map((group) => (
          <div key={group.key} className="flex flex-col gap-2">
            ...
            {group.pages.map((page) => {
              ...
              return (
                <div key={page.pageId} className="group rounded-md border border-border-ide bg-bg-panel/70 px-3 py-2 flex flex-col gap-1.5 transition-colors hover:bg-bg-panel hover:border-primary/30">
                  ...
                  {page.evidence && (
                    <div className="flex flex-col gap-1">
                      <button
                        type="button"
                        className="self-start rounded border border-border-ide px-2 py-1 text-text-secondary hover:text-text-primary hover:border-primary/40 transition-colors"
                        style={{ fontSize: '10px' }}
          ...
          {planRevisionRequested ? (
            <div className="flex flex-col gap-2">
              ...
              <textarea
                ...
                rows={3}
                className="w-full rounded-md border border-border-ide bg-bg-canvas px-2 py-1.5 text-text-primary resize-y outline-none focus:border-primary/50 focus:ring-1 focus:ring-primary/20"
                style={{ fontSize: '11px' }}
              />
              <div className="flex items-center gap-2 mt-1">
                <button
                  type="button"
                  className="px-2.5 py-1 rounded bg-primary text-white font-medium disabled:opacity-50 disabled:cursor-not-allowed transition-colors hover:bg-primary"
                  style={{ fontSize: '10px' }}
                ...
                <button
                  type="button"
                  className="px-2.5 py-1 rounded border border-border-ide text-text-secondary font-medium transition-colors hover:border-primary/40 hover:text-text-primary"
                  style={{ fontSize: '10px' }}
            ...
            <div className="flex flex-wrap items-center gap-3">
              ...
              <div className="flex items-center gap-2 shrink-0">
                <button
                  type="button"
                  className="h-6 px-3 rounded border border-transparent bg-primary text-white font-medium inline-flex items-center justify-center whitespace-nowrap transition-colors hover:bg-primary focus:outline-none focus:ring-1 focus:ring-primary/60"
                  style={{ fontSize: '11px' }}
                  onClick={onConfirm}
                >
