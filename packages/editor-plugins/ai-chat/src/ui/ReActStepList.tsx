...
import { Bot, CheckCircle2, CircleDashed, Wrench } from 'lucide-react';
          ...
          <div key={`${step.timestamp}-${step.stepIndex}`} className="border border-border-ide rounded-md bg-bg-panel/70 p-2.5 flex flex-col gap-2">
            <div className="flex items-start gap-2">
              ...
              <div className="min-w-0 flex-1">
                ...
                {(step.status || step.reasoningSummary) && (
                  <div className="text-text-secondary mt-1 whitespace-pre-wrap" style={{ fontSize: '11px' }}>
                    {step.status ?? step.reasoningSummary}
                  </div>
                )}
              </div>
            ...
            <div className="grid grid-cols-1 gap-2 md:grid-cols-2">
              <div className="rounded bg-bg-canvas px-2 py-1.5">
                ...
                <pre className="m-0 whitespace-pre-wrap break-all text-text-primary font-mono" style={{ fontSize: '10px' }}>
              ...
              <div className="rounded bg-bg-canvas px-2 py-1.5">
                ...
                <pre className={`m-0 whitespace-pre-wrap break-all font-mono ${step.error ? 'text-red-400' : 'text-text-primary'}`} style={{ fontSize: '10px' }}>
